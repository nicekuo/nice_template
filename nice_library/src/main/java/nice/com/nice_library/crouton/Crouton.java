...
package nice.com.nice_library.crouton;
 ...
 * within {@link Activity#onDestroy()} to avoid {@link Context}
 * leaks.
 */
public final class Crouton {
	 ...
	 *
	 * @param activity
	 *            The {@link Activity} that the {@link Crouton}
	...
	private TextView initializeTextView(final Resources resources) {
		...
		if (this.style.fontName != null) {
			...
			setTextWithCustomFont(text,
					resources.getString(this.style.fontNameResId));
		} else {
	...
	private void setTextWithCustomFont(TextView text, String fontName) {
		if (this.text != null) {
			...
			s.setSpan(new TypefaceSpan(text.getContext(), fontName), 0,
