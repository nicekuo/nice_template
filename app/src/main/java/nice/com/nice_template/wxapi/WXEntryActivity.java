package nice.com.nice_template.wxapi;
...
//        api.handleIntent(getIntent(), this);
    ...
    public void onResp(BaseResp baseResp) {
        ...
        NiceLogUtil.d("onResp$%^&*()$%^&*($%^&*($%^&*(#$%^&*()#$%^&*()$%^&*($%^&*(#$%^&*(#$%^&*(");

        if (baseResp instanceof SendAuth.Resp) {
            switch (baseResp.errCode) {
                case BaseResp.ErrCode.ERR_OK:
                    String code = ((SendAuth.Resp) baseResp).code;
                    NiceLogUtil.d("成功：   ===    code =   ”" + code);
                    sendBroadcast(code);
                    break;
                case BaseResp.ErrCode.ERR_USER_CANCEL:
                    ToastUtil.showToastMessage(WXEntryActivity.this, "您取消了授权");
                    break;
            }
            finish();
        } else {
    ...
    private void sendBroadcast(String code) {
        ...
        intent.putExtra(ConfigValue.kWeChatLoginKey, code);
        intent.setAction(ConfigValue.ActionWeChatLogin);
